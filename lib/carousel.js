...
var Carousel = (0, _createReactClass2['default'])({
  ...
  propTypes: {
    ...
    heightMode: _propTypes2['default'].oneOf(['max', 'adaptive']).isRequired,
    ...
    nextSlidesToLoad: _propTypes2['default'].number,
    prevSlidesToLoad: _propTypes2['default'].number
  },
  ...
  formatChildren: function formatChildren(children) {
    ...
    var nextSlidesToLoad = this.props.nextSlidesToLoad;
    var prevSlidesToLoad = this.props.prevSlidesToLoad;
    return _react2['default'].Children.map(children, function (child, index) {

      var nextSlideShouldLoad = false;
      if (self.props.lazyLoad && nextSlidesToLoad) {
        ...
        if (self.props.lazyLoad && self.state.currentSlide + nextSlidesToLoad > self.state.slideCount) {
          var breakpoint = self.state.currentSlide + nextSlidesToLoad - self.state.slideCount;
          nextSlideShouldLoad = index <= breakpoint;
        }
      }

      var prevSlideShouldLoad = false;
      if (self.props.lazyLoad && prevSlidesToLoad) {
        ...
        if (self.props.wrapAround && self.state.currentSlide - prevSlidesToLoad < 0) {
          var breakpoint = self.state.slideCount + self.state.currentSlide - prevSlidesToLoad;
  ...
  setDimensions: function setDimensions(props) {
    ...
    if (!frame || !frame.childNodes || !frame.childNodes[0]) {
      return null;
    }
